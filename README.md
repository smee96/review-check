...
### 2025-11-14 (v70)
- ✅ **관리자 캐페인 목록 과금 정보 표시 개선**
  - 새로운 과금 방식 정보 표시 (product_value, sphere_points, pricing_type)
  - 포인트 0원 캐페인도 과금 정보 표시
  - point_reward > 0 조건 제거
  - 과금 방식 라벨 표시 (포인트만/구매+포인트/상품만 등)

### 2025-11-14 (v69)
- ✅ **결제 완료 처리 버튼 표시 수정**
  - 포인트 0원 캐페인도 결제 버튼 표시
  - point_reward > 0 조건 제거
  - 모든 캐페인에 결제 완료 처리 가능
  - 고정 수수료 있는 캐페인 처리 가능
...
### 2025-11-14 (v67)
- ✅ **모집인원 변경 시 자동 재계산 추가**
  - 모집인원 입력 중 실시간 비용 재계산 (10명 이상일 때)
  - 포커스 아웃 시 최종 비용 재계산
  - 제출 전 항상 정확한 비용 계산 보장
  - 인원수와 비용 불일치 오류 방지
...
### 2025-11-14 (v65)
- ✅ **과금방식 변경 시 계산 버그 수정**
  - 숨겨진 필드의 값을 0으로 초기화하여 계산에서 제외
  - 포인트만 선택 시 상품 가치 0으로 리셋
  - 상품만/이용권만 선택 시 포인트 0으로 리셋
  - 다시 표시할 때 입력값 자동 복원
